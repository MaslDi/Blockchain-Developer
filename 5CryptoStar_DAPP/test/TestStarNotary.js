...
it('can add the star name and star symbol properly', async() => {
    let tokenId = 10;
    let instance = await StarNotary.deployed();
    await instance.createStar('Teststar', tokenId, {from: accounts[1]});
    let starInfo = await instance.tokenIdToStarInfo.call(tokenId);
    assert.equal(starInfo, 'Teststar');
    assert.equal(await instance.lookUptokenIdToStarInfo(tokenId), 'Teststar');
    assert.equal(await instance.name.call(), 'HansWurst');
    assert.equal(await instance.symbol.call(), 'HANS');
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let tokenId = 11;
    let instance = await StarNotary.deployed();
    await instance.createStar('CreatedStar', tokenId, {from: accounts[0]});
    // 2. Call your method lookUptokenIdToStarInfo
    // 3. Verify if you Star name is the same
    assert.equal(await instance.lookUptokenIdToStarInfo(11), 'CreatedStar');
});
